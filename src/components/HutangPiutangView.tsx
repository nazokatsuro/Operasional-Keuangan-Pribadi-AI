...
export function HutangPiutangView({
  ...
  const getDueAlertStatus = (dueDateStr: string, isLunas: boolean) => {
    ...
    if (diffDays < 0) {
      return { label: `Overdue ${Math.abs(diffDays)} Hari`, color: 'text-[#ff5c7a] bg-[#ff5c7a]/10 font-bold animate-pulse' };
    }
    if (diffDays === 0) {
      return { label: 'Jatuh Tempo Hari Ini!', color: 'text-amber-400 bg-amber-500/10 font-bold' };
    }
    if (diffDays <= 7) {
      return { label: `${diffDays} Hari Lagi`, color: 'text-[#ffb547] bg-white/5' };
    }
    return { label: `${diffDays} Hari Lagi`, color: 'text-[#9aa4bf] bg-white/5' };
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-white">Buku Hutang & Piutang</h1>
          <p className="text-xs text-[#9aa4bf]">Atur liabilitas penayangan cicilan jatuh tempo dan catat pinjaman keluar</p>
        </div>
        <button 
          onClick={handleOpenAdd}
          className="flex items-center gap-1.5 px-4.5 py-2.5 rounded-xl bg-[#7c5cff] text-white font-bold text-xs hover:bg-[#6847ff] hover:shadow-[0_0_15px_rgba(124,92,255,0.3)] active:scale-95 transition-all cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.06] flex items-center justify-between shadow-md">
          <div className="space-y-1">
            <span className="text-[10px] text-[#9aa4bf] font-mono tracking-widest uppercase block">Liabilitas Hutang Kita (Terbuka)</span>
            <span className="text-2xl font-black text-[#ffb547] block">{formatCurrency(totalHutang)}</span>
          </div>
          <UserMinus className="h-9 w-9 text-[#ffb547] ml-4 bg-amber-500/10 p-2 rounded-xl" />
        </div>

        <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.06] flex items-center justify-between shadow-md">
          <div className="space-y-1">
            <span className="text-[10px] text-[#9aa4bf] font-mono tracking-widest uppercase block">Saku Piutang Keluar (Terbuka)</span>
            <span className="text-2xl font-black text-[#16c784] block">{formatCurrency(totalPiutang)}</span>
          </div>
          <UserPlus className="h-9 w-9 text-[#16c784] ml-4 bg-emerald-500/10 p-2 rounded-xl" />
        </div>
      </div>
        ...
        {debts.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Footer row displaying dates and reminders */}
              <div className="flex flex-wrap gap-2 items-center justify-between mt-5 pt-3.5 border-t border-white/[0.03] text-xs">
                <div className="flex items-center gap-1.5">
                  ...
                  {!isLunas && (
                    <button
                      onClick={() => handleOpenPayInstallment(item)}
